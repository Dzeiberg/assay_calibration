...
def sn_method_of_moments_init(X):
    ...
    try:
        ...
        if np.any(np.isnan([mu, sigma, lambda_init])) or np.any(
            np.isinf([mu, sigma, lambda_init])
        ):
...
def methodOfMomentsInit(X, n_components, constrained, max_attempts=1000, **kwargs):
    ...
    for attempt in range(max_attempts):
        if np.random.rand() < 0.7:  # 70% of the time use percentile-based
            ...
            cutPoints = np.percentile(
                X, np.sort(np.clip(base_percentiles + jitter, 1, 99))
            )
        ...
        for i in range(n_components):
            ...
            min_samples = max(
                10, int(0.05 * len(X))
            )  # Require at least 5% of data per component intitialization
...
def fix_to_satisfy_density_constraint(component_parameters, xlims, **kwargs):
    ...
    if n_components == 3:
        ...
        for _ in range(300):
            v01 = density_constraint_violated(
                component_parameters[0], component_parameters[1], xlims
            )
            v12 = density_constraint_violated(
                component_parameters[1], component_parameters[2], xlims
            )
            ...
            if param_to_adjust == "skew":
                ...
                if v12:
                    ...
                    component_parameters[2] = [
                        ...
                        + 0.05 * abs(component_parameters[2][0]),
        ...
        if density_constraint_violated(
            component_parameters[0], component_parameters[1], xlims
        ) or density_constraint_violated(
            component_parameters[1], component_parameters[2], xlims
        ):
            return [[] for _ in range(n_components)]
    else:
