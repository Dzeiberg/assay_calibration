...
def tryToFit(observations, sample_indicators, num_components, constrained, init_method, init_constraint_adjustment, **kwargs)->Dict:
...
def get_sample_cdf(component_params, weights, scores, sampleNum):
    """
    ...
    """
    if sampleNum >= weights.shape[0] or sampleNum < 0:
        raise ValueError(
            f"Sample number {sampleNum} must be greater than 0 and less than the number of samples {weights.shape[0]}."
        )
    return np.stack(
        [
            weights[sampleNum, compNum]
            * skewnorm.cdf(scores,params[0],loc=params[1],scale=params[2])
            for compNum,params in enumerate(component_params)
        ],
        axis=0,
    ).sum(axis=0)
...
class Fit:
    ...
    @classmethod
    def from_dict(cls, scoreset, fit_result):
        cls = cls(scoreset)
        cls.fit_result = fit_result
        cls._eval_metrics = fit_result.get("eval_metrics",{})
        return cls
    
    @property
    def bootstrap_seed(self):
        return self._bootstrap_seed

    @bootstrap_seed.setter
    def bootstrap_seed(self, value):
        self._bootstrap_seed = value
    ...
    def run(self, component_range, **kwargs):
        ...
        if core_limit == 1:
            ...
            models = Parallel(n_jobs=core_limit, batch_size=1, verbose=verbosity)( # type: ignore
                delayed(tryToFit)(
                    train_observations,
                    train_sample_assignments,
                    num_components,
        ...
        if kwargs.get("bootstrap", True):
            val_lls = [get_likelihood(val_observations,
                                      val_sample_assignments,
                                      m['component_params'],
                                      m['weights']) / len(val_sample_assignments) for m in models]
    ...
    def generate_fit_jobs(self, component_range, save_dir: str|Path, **kwargs):
        """
        ...
        Required Arguments
        --------------------------
        - component_range: List[int] : number of components to use when trying to fit models
        - save_dir: str|Path : directory to save fit results in
        ...
        for i in range(NUM_FITS):
            for num_components in component_range:
                job = {
                    ...
                    'kwargs': kwargs.copy(),
                    'scoreset_name': self.scoreset.scoreset_name
                }
                jobs.append(job)
    ...
    def execute_fit_job(job):
        ...
        save_path = f"{job['save_dir']}/{job['scoreset_name']}_b{job['bootstrap_seed']}_c{job['num_components']}_f{job['fit_idx']}.json"
        ...
        try:
            ...
            # Save to disk
            save_data = {
                'scoreset_name': job['scoreset_name'],
            ...
            save_data = serialize_dict(save_data)
            
            with open(save_path, 'w') as f:
                json.dump(save_data, f,indent=4)
            
        except Exception as e:
            print(f"Failed: {job['scoreset_name']} b{job['bootstrap_seed']} c{job['num_components']} f{job['fit_idx']}: {e}")
            raise e
